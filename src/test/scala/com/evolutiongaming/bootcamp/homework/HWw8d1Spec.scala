...
class MonadsSpec extends PropSpec with ScalaCheckPropertyChecks with Matchers {
  ...
  property("Maybe") {
    ...
    def isEven(x: Int): Maybe[Int] = if (x % 2 == 0) { Just(x) } else { None }

    forAll { (x: Int, y: Int, z: Int) => {
       leftID(x)(isEven) should be (isEven(x))
       rightID(isEven(x)) should be (isEven(x))
       def f(a: Int): Maybe[Int] = if (x % 2 == 0) { isEven(a) } else { None }
       def g(a: Int): Maybe[Int] = if (y % 2 == 0) { isEven(a) } else { None }
